...
##Usage: TreeOTU.pl -i input_rooted_tree_in_newick_format -c PN_cutoff(0<=cutoff<1) -o output_file

###input format:
### -i inputtree, must be in newick format
### -c PN cutoff, must >=0 and <1
##  -o output file, of no output file define, the script output to STDOUT


my %opt=@ARGV;
my $opt_i=$opt{'-i'}; ## input tree
my $opt_c=$opt{'-c'}; ## depth cutoff from tip, must be >=0 and <1
my $opt_o=$opt{'-o'}; ## output file
my @t=split(/\//,$opt_i);
$t[-1]=~s/\s+/_/g;
my $output=$t[-1]."_PN_cutoff_".$opt_c."\t";


my $tree;

open(I,$opt_i)||die "cannot open tree file $opt_i\n";
while(<I>){
    $tree.=$_;
}
close I;
...
my $tree_obj=TREE_MOD->new($tree);

my $otu=$tree_obj->OTU_by_normalized_depth_from_tip_weighted($opt_c);
@t=split(/\n/,$otu);
foreach my $t(@t){$t=~s/\s+/,/g;}


if($opt_o){
open(OUT,">$opt_o") || die "cannot create file $opt_o \n";
print OUT $output.join("\t",@t)."\n";
close OUT;
}
else{
print $output.join("\t",@t)."\n";
...
my %leaf_to_node_depth;
    ...
    foreach my $t(keys %{$class->{parent}}){
        my $p=$class->{parent}->{$t};
        if( length($p) && length($t) ){$child{$p}->{$t}=1;}
        if($t=~/^T/){push (@proc,$t);
...
foreach my $t(keys %child){
   ...
   if($t_count>=1 && $n_count==0 ){$next_proc{$t}=1;}

   if($t_count==0 && $n_count==0) {
    $childless++;    
    delete $child{$t};
    my $this_p=$class->{parent}->{$t};
    if($this_p){delete $child{$this_p}->{$t};}
     } ##end if t=0 n=0
  }  ##end $t
 

}#end childless while loop

   
foreach	my $t(keys %next_proc){push(@proc,$t);}
