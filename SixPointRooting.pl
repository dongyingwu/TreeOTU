#!/usr/bin/perl
##Usage:SixPointRooting.pl -i input_tree -l taxa_list_of_six -o out_tree
...
##input tree
open(I,$opt_i)||die "cannot open file $opt_i \n";
while(<I>){
$tree.=$_;
}
close I;
...
open(I,$opt_l)||die "cannot open list file $opt_l\n";
while(<I>){
$_=~s/^\s+//;
my @t=split(/\s+/,$_);
@tx=(@tx,@t);
...
$tree=TREE_MOD->new($tree);

$tree->six_point_rooting(@tx);
